...
/* 
   TP1 - Entrega final
   Comisión: Matias
   Alumnas: Luciana Tabassi; Zegarra Aylen; Sanchez Lourdes.
*/
...
float umbralRuido = 350;

float umbralRuidoMin = umbralRuido;
float umbralRuidoMax = 800;
...
void draw () {
  ...
  if (haySonido) {
    ...
    if (momentoActual > marcaDeTiempo + umbralTiempo2) {
      ...
      if (bajoYgrave) {
        t.trazos(paleta.darColorPaletaDos(200),  200, 300, 200);  //
